...
class GuildDatabase extends dblib.Database {
     ...
     * @param keycolumn {String} - the name of the key
     * @param keyvalue {*} - the value of the key
     * @returns {Promise<*>}
     */
    async getValueByKey(table, column, keycolumn, keyvalue) {
        let result = await this.get(this.sql.select(table, false, valuecolumn,
    ...
    /**
     * Sets a value column to a value with a key column.
     * @param table
     * @param valuecolumn
     * @param value
     * @param keycolumn
     ...
     */
    async setValueForKey(table, valuecolumn, value, keycolumn, keyvalue) {
        let row = await this.getValueByKey(table, valuecolumn, keycolumn, keyvalue);
        let newRow = {};
        newRow[valuecolumn] = this.sql.parameter(1);
        if (!row || Number(row.count) === 0) {
            newRow[keycolumn] = this.sql.parameter(2);
            await this.run(this.sql.insert(table, newRow),
                [value, keyvalue]);
        } else {
            await this.run(this.sql.update(table, {value: this.sql.parameter(1)},
                this.sql.where(this.sql.parameter(2), '=', this.sql.parameter(3))), [value, keycolumn, keyvalue]);
        }
    }
     ...
     * @param [limit] {Number} - the limit of rows to get
     * @returns {Promise<void>}
     */
    ...
    /**
     * Returns all saved media entries.
     * @returns {Promise<void>}
     */
    async getMediaEntries() {
        return this.getTableContent('savedmedia');
    }
